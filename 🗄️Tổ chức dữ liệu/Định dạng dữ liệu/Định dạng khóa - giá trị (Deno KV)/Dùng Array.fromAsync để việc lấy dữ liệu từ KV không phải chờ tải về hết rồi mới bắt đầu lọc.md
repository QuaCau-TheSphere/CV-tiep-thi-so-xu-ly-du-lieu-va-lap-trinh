...
deno eval --unstable-kv "const kv = await Deno.openKv(); const all = await Array.fromAsync(kv.list({prefix:[]})); console.log(all)"
```
...
[[Nếu không để đường dẫn vào Deno.openKv(), thì mặc định là ở DENO_DIR. Ở local thì dùng deno info để xem]]
