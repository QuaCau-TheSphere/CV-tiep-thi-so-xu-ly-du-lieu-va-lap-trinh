...
Nên dùng `git log --oneline --graph` trước để xem trước ID của một số commit có trước commit lỗi.
...
```
add-Content .\.gitignore .obsidian/plugins/obsidian-mkdocs-publisher/logs.txt 
git rm --cache -r . && git add -A && git commit --amend --no-edit && git rebase --continue && git add -A && git rebase --continue
```
### `git commit --amend`
###  `git rebase --continue`
